...
def mvee(points, tol=0.00001):
    ...
    while err > tol:
        ...
        A = np.dot(np.dot(Q.T, la.inv(X)), Q)
        # print(A)
        # print(np.diag(A))
...
def mvee2(points, tolerance=0.0001):
    """
    Finds the ellipse equation in "standard form"
    x.T * A * x = 1
    """
    ...
    while error > tolerance:
        X = Q @ np.diag(weights) @ Q.T
        Mahalanobis_distances = np.diag(Q.T @ la.inv(X) @ Q)
        farthest_point_index = np.argmax(Mahalanobis_distances)
        step_size = (Mahalanobis_distances[farthest_point_index] - dimension - 1.0) / ((dimension + 1) * (Mahalanobis_distances[farthest_point_index] - 1.0))
        new_weights = (1 - step_size) * weights
        new_weights[farthest_point_index] += step_size
        error = la.norm(new_weights - weights)
        weights = new_weights
    
    covariance_inverse = la.inv(np.dot(np.dot(points.T, np.diag(weights)), points)) / dimension
...
vectors = np.array([[1, 2, 4, 6], [2, 1, 2, 4], [2, 5, -2, -3], [6, -7, 3, 2], [100, 30, 40, -100],[200, 60, 80, -200]])
# vectors = np.array([[1, 2, 4, 6], [2, 1, 2, 4]])

# print(np.concatenate([vectors, -vectors]))
# print(la.matrix_rank(vectors))

vectors = 1* vectors

# print(mvee2(np.concatenate([vectors, -vectors]))/ 1.25)
A1, c = mvee(np.concatenate([vectors, -vectors]))
A2 = mvee2(np.concatenate([vectors, -vectors]))
print(la.det(A1), la.det(A2))

m = max(v.T @ A1 @ v for v in vectors)
# A = A / m
for v in vectors:
    print(v.T @ A1 @ v, v.T @ A2 @ v)
# print(c)
print(A1)
print(A2)
